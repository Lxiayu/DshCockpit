...
// filesystem, no network. The schema-v1 validation rules (parseDraft /
// validateDraftSchema / ASSET_BY_ID) live in layout-schema.js — the single
// source of truth shared with the production boot chain (P5/B-1 extraction,
// docs/strategy/2026-09-23-p5-deletion-inventory.md §3); this file only adds
// the editing state machine on top of them. Import is atomic: any validation
// failure leaves the current draft, selection and undo/redo history
// untouched.
//
// toJSON whitelists id/kind/asset/position/scale/direction/layer/groupId/
// locked/hidden/name per item and deep-copies everything; auto-generated
// draft-N ids continue past imported draft-N ids.

// P5/B-1 (2026-09-23): the schema-v1 validation slice (parseDraft,
// validateDraftSchema) plus the catalog-derived tables it needs live in
// layout-schema.js — the production boot chain depends on them (Task 8), so
// they must not ship inside the editor core. Single source of truth.
const {
  parseDraft,
  validateDraftSchema: validateDraftSchemaProbe,
  SCHEMA_VERSION,
...
function createLayoutEditor({ scene }) {
  ...
  // priority chain (office-boot.resolveProductionLayoutDraft) — P5/B-1 moved
  // the implementation to layout-schema.js (shared with the boot chain); the
  // editor API keeps the same method name over the same behaviour.
  function validateDraftSchema(draft) {
    return validateDraftSchemaProbe(draft);
  }
