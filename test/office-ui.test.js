...
test('real-shell probes and the editor launcher never flash windows (M2b 2026-09-16)', () => {
  ...
  // default for real users. The launcher keeps running the production
  // office.html until the editor page lands (P5 B-2 re-points its URL).
  ...
  // This file's own probe surface: the inline probes left with the editor
  // test suite (P5) — the remaining real-shell tests spawn the shared
  // templates above (show:false + app.dock + backgroundThrottling, already
  // asserted per template). What must stay true for THIS file: it creates
  // no inline BrowserWindow at all and never shows one.
  const self = fs.readFileSync(__filename, 'utf8');
  assert.doesNotMatch(self, /show:\s*true/, 'no probe may show a window');
  assert.doesNotMatch(self, /new BrowserWindow\(/, 'no inline probe window remains — real-shell probes run the shared templates');
  // P5 note: E2d (the launcher-driven editor save/relaunch/restore round
  // trip) left with the editor UI itself — it returns with the editor page
  // (P5 B-2). The launcher's headless-env contract stays asserted above.
});
...
test('E5a-R2 real shell: composed character scale and facing reach the runtime', { timeout: 420000 }, () => {
  ...
  assert.ok(fs.existsSync(resultsFile), 'the probe wrote its results file');
  const summary = JSON.parse(fs.readFileSync(resultsFile, 'utf8'));
  assert.equal(summary.layoutSource, 'saved-compiled', 'the saved user draft drives the runtime');
  assert.equal(summary.draftCharacter.asset, 'whale-girl-back');
  // the height mapping (documented in office-module.js): the composed art
  // height is DRAFT_WIDTHS.character × scale × unionH / packCanvasW and the
  // renderer's visibleHeight IS that composed height (delivered as a ratio
  // over the linear default, clamped to the SPEC-02 band). The editor used
  // to be cross-measured against this; P5 removed the editor from the page,
  // so the runtime height is now pinned against the documented formula
  // itself — the same quantity, without the editor surface.
  const unionH = summary.packGeometry.visibleBounds.height;
  const packCanvasW = summary.packGeometry.outputCanvas.width;
  const expectedVisibleHeight = 96 * summary.draftCharacter.scale * unionH / packCanvasW;
  assert.ok(Math.abs(summary.runtimeIdle.visibleHeight - expectedVisibleHeight) <= 0.5,
    `composed height: runtime ${summary.runtimeIdle.visibleHeight.toFixed(2)}px vs formula ${expectedVisibleHeight.toFixed(2)}px (Δ ${Math.abs(summary.runtimeIdle.visibleHeight - expectedVisibleHeight).toFixed(2)}px)`);
  ...
  // the review artifacts exist (runtime-only since P5 removed the editor)
  for (const name of ['runtime-idle.png', 'runtime-working.png']) {
    assert.ok(fs.existsSync(path.join(evidenceDir, name)) && fs.statSync(path.join(evidenceDir, name)).size > 1000, `${name} captured`);
  }
  // side-by-side composites for the human review (idle vs working)
  const compose = `
from PIL import Image
...
pair = Image.new('RGB', (rt2.width + wk2.width + 12, h), (255, 255, 255))
pair.paste(rt2, (0, 0)); pair.paste(wk2, (rt2.width + 12, 0))
pair.save('/tmp/e5a-r2-evidence/side-by-side-idle.png')
pair2 = Image.new('RGB', (rt2.width + wk2.width + 12, h), (255, 255, 255))
pair2.paste(rt2, (0, 0)); pair2.paste(wk2, (rt2.width + 12, 0))
