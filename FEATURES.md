...
### 5. 虚拟办公室（v0.4.0）
- **真实事件驱动**：harness 的 turn / 工具调用 / 子代理起止 / 终态经适配层进入单一逻辑时钟的仿真——开工走向工位、落座打字、交付或出错的仪式都由真事件决定，不是定时动画；
- **子代理座位**：调度员之外的四个座位（研究员 / 工程 / 评审 / 协作者）按真实子代理角色分类上工（分类只读 `label/phase/mode` 这类有界元数据，判不确定即按协作者处理）；子代理会话按 `{kind:'subagent', parentSessionId, childSessionId, mode}` 寻址 follow/page（0.1.5 拒绝根地址），并有「连开 N 次零帧即停」的有界守卫（`src/office/runtime/follow-address.js`）；
- **右栏六块信息架构**：今日用量（token/金额/缓存节省/预算）/ 员工行 / 待你处理 / 时间线 / 选中员工详情 / 页脚；逐 turn token 归属取自 harness 每轮的 usage 记录（非日桶差值）；
- **审批收件箱**：低/中危行内批拒；高危走危险模态（步骤、影响、**真实命令原文**、可否撤销、**无「总是允许」**）；回答复用与 IM 同一条 `$events/result` 通道；
- **隐私边界**：气泡与面板只吃事实（工具名转中文短语、计数、终态、token 数字）——提示词、代码、路径原文与模型输出不进办公室；
- **后台存活与恢复**：办公室不是当前主视图时仿真不冻结；切回前台时有界恢复（先降画质档、末档才静止、每次激活一次恢复预算 + 手动重试），渲染态变化写运行日志；
- **启动默认视图**：0.4.0 起打开应用先看到办公室（用户拍板 M6 P5.2；`DSH_DESKTOP_OPEN_OFFICE=0` 可回退到"会话工作台优先"，设置项后续补）
...
- **1393 项单元测试**（node --test）、日志轮转、存储管理（占用可视化 + 一键清理）、崩溃诊断记录；
