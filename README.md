...
- **手机远程控制**：设置中开启后，手机连同一 Wi-Fi 扫码配对，即可在手机浏览器完整使用 dsh Web UI——查看会话流式输出、发消息/插话、**审批工具执行、回答提问**。自签 HTTPS + 一次性配对码 + 加密长期令牌；运行时本身仍只监听 127.0.0.1，局域网只能到达壳的鉴权网关（详见 DESIGN.md §17）。
