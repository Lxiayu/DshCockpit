...
发布阻塞项修复轮（Task 9 blocker fix round）：Blocker A（SPEC-07 缺包启动失败）、Blocker B（SPEC-07 运行期低 FPS 降级）、Blocker C（npm test 全量挂起）修复后的重新验收； SPEC-09 自动化门、Electron 场景矩阵、性能/视觉/隐私门。证据基线：feature/s1-office @ 2194abc（工作区用户既有 dirty 文件未纳入）。证据 ID：2194abc-task9-fix1（上一轮归档：b349adf-task9）。
...
- `node --check src/window-manager.js` → exit 0 (passed, 19ms) ok
- `for file in src/office/runtime/*.js; do node --check "$file"; done` → exit 0 (passed, 282ms) ok
- `git diff --check` → exit 0 (passed, 11ms) ok
- `npm test` → exit 0 (passed, 2104ms) 
- `node --test test/office-*.test.js` → exit 0 (passed, 437ms) node:test spec counts approximated: pass~477 fail~0
...
- npm test 全量 runner：已退出，exit 0；等价的逐文件运行见 npmtest-perfile.json，失败项：none — every repo test file passes when run individually
- `git diff --check` exit 0；工作区用户既有修改未 stage、未回滚

## Evidence

全部位于 artifacts/office/task9/2194abc-task9-fix1/（脱敏后）：commands.txt、result.json、diagnostics.json、performance.json、replay.json、manual-review.json、npmtest-perfile.json、report.md，以及 22 张固定视口截图（S1 局部漫游、S2 到岗/工作/结果、S3 FIFO、S4 cancel、S5 stale、S6 窄窗口、S7 双 view、S8 两种 fallback、S9 恢复后页面、hidpi、perf、replay-a/b）。运行时衍生数据在装配前经过 shared privacy redactor（redacted 模式），装配结果另经独立模式扫描：无绝对路径、无 Session/Run ID、无密钥形态。
